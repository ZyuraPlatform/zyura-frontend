...
import { ArrowLeft, CheckCircle, Clock, MinusCircle, XCircle } from "lucide-react";
...
export default function StudyPlanView({
  ...
  isLoading,
  backTo,
  emptyTitle = "Study Plan Not Found",
  emptyDescription = "The requested study plan could not be found.",
}: Props) {
  ...
  const handleStart = async (taskType: string, contentId: string, dayNumber: number) => {
    if (!contentId) {
  ...
  const totalTasks = studyPlan.daily_plan.reduce((sum, d) => sum + d.hourly_breakdown.length, 0);
                          ...
                          {dayPlan.hourly_breakdown.map((session, idx) => (
                            ...
                            >
                              <div className="mb-2 sm:mb-0">
                                <h4 className="font-semibold text-gray-900 text-base">{session.description}</h4>
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  handleStart(session.task_type, session.suggest_content?.contentId, dayPlan.day_number)
                                }
